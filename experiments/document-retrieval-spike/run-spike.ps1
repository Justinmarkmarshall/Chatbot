...
try {
    ...
    $preserved = @(Get-ChildItem ../embedding-spike,../pgvector-spike -Recurse -File |
        Where-Object { $_.FullName -notmatch '\\(bin|obj|models|\.venv|__pycache__)\\' } |
        ForEach-Object { [pscustomobject]@{
            Path=$_.FullName.Substring($repositoryRoot.Length + 1)
            Sha256=(Get-FileHash -LiteralPath $_.FullName -Algorithm SHA256).Hash
        } })
    $preserved | ConvertTo-Json -Depth 3 | Set-Content results/preserved-before.json
    ...
    [ordered]@{
        ...
        NetworkInspection=(docker network inspect chatbot-document-retrieval-spike_spike --format '{{.Internal}}')
    ...
    if ($null -ne $preserved) {
        $changed = @($preserved | Where-Object {
            $path = Join-Path $repositoryRoot $_.Path
            !(Test-Path -LiteralPath $path) -or (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash -ne $_.Sha256
        })
        [ordered]@{TimestampUtc=[DateTime]::UtcNow;CheckedFiles=$preserved.Count;AllUnchanged=($changed.Count -eq 0);Changed=$changed} |
            ConvertTo-Json -Depth 5 | Set-Content results/preservation-check.json
        if ($changed.Count -gt 0) { Write-Warning 'Previous experiment files changed during this run; inspect the preservation audit.' }
    }
    Pop-Location
}
