...
The restore step runs NuGet directly. For `GITHUB_TOKEN`, inspect **Set up job > GITHUB_TOKEN Permissions** for `Packages: read`. In the package's **Package settings > Manage Actions access**, grant `Justinmarkmarshall/Chatbot` Read access (this is separate from linking its source repository or granting Codespaces access).
...
The fallback is used consistently for test restore and the Docker BuildKit NuGet secret. GHCR publishing still uses `GITHUB_TOKEN`. Without the optional secret, package reads still use `GITHUB_TOKEN`; no PAT is mandatory. Fork PRs do not receive repository secrets. Push the workflow change and start a run for that new commit; rerunning an old commit will not pick up the new YAML. Live credential validation requires that Actions run.
